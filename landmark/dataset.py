...
from mmfashion.datasets.registry import DATASETS


def get_img_tensor(img_path, get_size=False):
...
class LandMarkDataset(data.Dataset):
    def __init__(self, dataroot, datamode, debug=False):
        # self.root = dataroot
        # self.mode = datamode
        self.img_root = op.join(dataroot, '{}_img').format(datamode)
    ...
    def __getitem__(self, idx):
        img_path = op.join(self.img_root, self.img_list[idx])
        # img_path = op.join(self.root, self.mode, self.img_list[idx])
        img_ten, img_w, img_h = get_img_tensor(img_path, True)
        return img_ten, img_w, img_h, self.img_list[idx]


# @DATASETS.register_module
class LandmarkDetectDataset(data.Dataset):
    CLASSES = None
    ...
    def get_basic_item(self, idx):
        ...
        if self.with_bbox:
            ...
            x1, x2 = 0, self.img_size[0]
            y1, y2 = 0, self.img_size[1]
